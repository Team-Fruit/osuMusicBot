...
class MusicBot(discord.Client):
    def __init__(self, config_file=ConfigDefaults.options_file, perms_file=PermissionsDefaults.perms_file):
        ...
        self.autoplaylist = song_list(self.config.osu_dir)
    ...
    async def on_player_finished_playing(self, player, **_):
        if not player.playlist.entries and not player.current_entry:
            while self.autoplaylist:
                song_path = choice(self.autoplaylist)
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                np_text = "Now Playing: **%s** added by **%s** %s\nURL: %s\n" % (
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str, player.current_entry.ur)
            else:
